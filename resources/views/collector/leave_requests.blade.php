...
<head>
    ...
    <title>Leave Request - SmartBin Collector</title>
    <!-- Google Fonts -->
...
<body class="bg-slate-50 text-slate-808 min-h-screen flex flex-col justify-between">
    <header class="bg-emerald-950 text-white sticky top-0 z-40">
            ...
            <div class="flex items-center space-x-8">
                ...
                <nav class="hidden md:flex space-x-6 text-sm font-semibold">
                    <a href="{{ route('waste_collector.dashboard') }}"
                        class="text-slate-350 hover:text-white">Dashboard</a>
                    <a href="{{ route('waste_collector.history') }}" class="text-slate-355 hover:text-white">Completed
                        Pickups</a>
                </nav>
            </div>
    ...
    <main class="flex-grow max-w-4xl w-full mx-auto px-4 py-8 space-y-6">
        <h2 class="text-3xl font-black text-slate-900 border-b pb-4">Submit Leave Request</h2>
        <div class="bg-white border rounded-3xl p-8 shadow-sm text-center py-16 space-y-4">
            <div class="w-16 h-16 rounded-full bg-rose-100 text-rose-600 flex items-center justify-center mx-auto"><i
                    data-lucide="mail" class="w-8 h-8"></i></div>
            <h3 class="text-xl font-bold text-slate-905">Leave Application Request</h3>
            <p class="text-slate-500 text-sm max-w-sm mx-auto">Request shift leaves and off-duty assignments directly to
                the SmartBin administration.</p>
        </div>
    </main>
