...
<head>
    ...
    <title>Create System Account - SmartBin</title>
    <!-- Google Fonts -->
...
<body class="bg-slate-50 text-slate-800" x-data="{ sidebarOpen: false, userRole: 'user' }">
        ...
        <div class="flex-grow flex flex-col h-screen overflow-y-auto">
            ...
            <header class="h-16 bg-white border-b border-slate-200/80 px-6 flex justify-between items-center shrink-0">
                <div class="flex items-center space-x-4">
                    ...
                    <h2 class="text-lg font-bold text-slate-850">Create User Profile</h2>
                </div>
            </header>
            ...
            <main class="flex-grow p-6 md:p-8 max-w-3xl w-full mx-auto space-y-8">
                ...
                <div class="mb-4">
                    ...
                    <h2 class="text-3xl font-black text-slate-900">Add Account Profile</h2>
                    <p class="text-slate-550 text-sm mt-1">Register a citizen user or create credentials for waste
                        collector staff.</p>
                </div>
                    ...
                    <form method="POST" action="{{ route('admin.users.store') }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <select name="role" required x-model="userRole"
                                class="w-full px-4 py-3.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none text-slate-800 text-sm transition-all bg-slate-50 font-semibold cursor-pointer">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-sm font-semibold text-slate-700 mb-2">Account Password</label>
                                <input type="password" name="password" required placeholder="Minimum 6 characters"
                                    class="w-full px-4 py-3 rounded-xl border border-slate-202 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none text-slate-850 text-sm font-semibold transition-all bg-slate-50">
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-slate-700 mb-2">Phone Number</label>
                                <input type="text" name="phone" required value="{{ old('phone') }}"
                                    placeholder="e.g. 01712345678"
                                    class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-505 outline-none text-slate-800 text-sm font-semibold transition-all bg-slate-50">
                            </div>
                        </div>
                        ...
                        <div class="space-y-6 pt-4 border-t border-slate-100">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                <div>
                                    <label
                                        class="block text-sm font-semibold text-slate-750 mb-2 flex items-center space-x-2">
                                    ...
                                    <select name="sector"
                                        class="w-full px-4 py-3.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none text-slate-800 text-sm transition-all bg-slate-50 font-semibold cursor-pointer">
                                        <option value="" disabled selected>Select Sector zone</option>
                                        @foreach($sectors as $sec)
                                            <option value="{{ $sec }}" {{ old('sector') == $sec ? 'selected' : '' }}>
                                                {{ $sec }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="road" placeholder="e.g. Road 12" value="{{ old('road') }}"
                                        class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none text-slate-805 text-sm font-semibold transition-all bg-slate-50">
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="address" rows="2" placeholder="Floor No, Flat details, Landmarks etc."
                                    class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-505 outline-none text-slate-808 text-sm font-semibold transition-all bg-slate-50">{{ old('address') }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="pt-6 border-t border-slate-100 flex items-center justify-end space-x-4">
                            ...
                            <button type="submit"
                                class="px-8 py-3 bg-emerald-600 hover:bg-emerald-550 text-white font-bold rounded-2xl transition-all shadow-md text-sm">
