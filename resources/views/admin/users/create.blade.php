...
@section('content')
    ...
    <div class="mb-8">
        <h1 class="text-[28px] font-black tracking-tight text-gray-900 mb-1">User & Staff Directory</h1>
    ...
    <div class="max-w-4xl mx-auto px-6 md:px-16">
        <!-- Registration Form -->
        <div
            class="bg-white rounded-3xl p-10 border border-gray-100 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] w-full mx-auto">
            ...
            <form method="POST" action="{{ route('admin.users.store') }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2 mb-4">
                        <label for="role" class="block text-sm font-bold text-gray-700 mb-2">Assign Operational Role</label>
                        <select id="role" name="role" required
                            ...
                            <option value="waste_collector" {{ old('role') == 'waste_collector' ? 'selected' : '' }}>Waste
                                Collector (Field Staff)</option>
                            <option value="user" {{ old('role') == 'user' ? 'selected' : '' }}>General User (Resident)
                            </option>
                        </select>
                    ...
                    <div>
                        ...
                        <input id="phone" type="text" name="phone" value="{{ old('phone') }}" required placeholder="+1 234 567 8900"
                            class="w-full bg-gray-50 border border-gray-200 text-gray-800 text-sm rounded-xl focus:ring-[#19817E] focus:border-[#19817E] block p-3 transition">
                    ...
                    <div>
                        ...
                        <select id="sector" name="sector" required
                            ...
                            @for($i = 1; $i <= 18; $i++)
                                <option value="Sector {{ $i }}" {{ old('sector') == "Sector $i" ? 'selected' : '' }}>Sector
                                    {{ $i }}</option>
                            @endfor
                        </select>
                    ...
                    <div>
                        ...
                        <input id="road" type="text" name="road" value="{{ old('road') }}" required
                            placeholder="e.g. Road 5, Block B, House 12"
                            class="w-full bg-gray-50 border border-gray-200 text-gray-800 text-sm rounded-xl focus:ring-[#19817E] focus:border-[#19817E] block p-3 transition">
                ...
                <div class="flex items-center justify-end mt-8 pt-6 border-t border-gray-100 gap-4">
                    <button type="submit"
                        class="bg-[#19817E] text-white text-sm font-bold py-3 px-8 rounded-xl hover:bg-teal-700 focus:ring-4 focus:ring-teal-100 transition shadow-sm">
                        Submit Registration
                    </button>
                </div>
            </form>
